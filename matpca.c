...
MATSTACK mat_pca(MATRIX data, int pca_type)
{
    ...
    tred2(symmat, evals, interm);  /* Triangular decomposition */
    tqli(evals, interm, symmat);   /* Reduction of sym. trid. matrix */
    ...
    for (j=0; j<m; ++j)
    {
        ...
        for (i=0; i<3; ++i)
        {
            ...
            if (evals[m-i-1][0] > 0.0005)   /* Guard against zero eigenvalue */
                symmat2[j][i] /= (mtype)sqrt(evals[m-i-1][0]);   /* Rescale */
            else
...
MATSTACK mat_eig_sym(MATRIX symmat, MATSTACK result)
{
    ...
    tred2(tmp_result1, tmp_result0, interm);  /* Triangular decomposition */
    tqli(tmp_result0, interm, tmp_result1);   /* Reduction of sym. trid. matrix */
...
MATSTACK mat_corcol(MATRIX data)
{
    ...
    for (j = 0; j < m; ++j)
    {
        ...
        for (i = 0; i < n; ++i)
        {
            stddev[0][j] += (( data[i][j] - corr[0][0][j] ) *
                             ( data[i][j] - corr[0][0][j] )  );
        }
        stddev[0][j] /= (mtype)n;
