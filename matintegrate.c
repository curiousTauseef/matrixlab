...
mtype mat_int_qadrat(mtype(*fx)(mtype), mtype lower, mtype upper)
{
    ...
    *x=upper ;
    f14=(*fx)(*x);
    result = __mat_lint(x, fx, lower, upper, f0, f2, f3, f5, f6, f7, f9, f14, hmin, hmax, re, ae)*16.0f;
    return result;
}

mtype __mat_lint(mtype *x, mtype (*fx)(mtype), mtype x0, mtype xn, mtype f0, mtype f2, mtype f3, mtype f5, mtype f6, mtype f7, mtype f9, mtype f14, mtype hmin, mtype hmax, mtype re, mtype ae)
{
    ...
    else
    {
        ...
        *x=xn-6.0f*h;
        ...
        return ((fabs(v-w)<fabs(v)*re+ae) ? h*v :
                (__mat_lint(x,fx,x0,xm,f0,f1,f2,f3,f4,f5,f6,f7,hmin,hmax,re,ae)-
                 __mat_lint(x,fx,xn,xm,f14,f13,f12,f11,f10,f9, f8, f7,hmin,hmax,re,ae)));
    }
}
